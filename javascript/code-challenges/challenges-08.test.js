...
const isCapitalized = (str) => {
  let verify = /\b[A-Z][a-z]*w\g/;
  return str.match (verify) || [];
};
...
const citiesAtoJ = (arr) => {
  let verify = /^[A-J]/;
  let result = [];
  arr.forEach(value => verify.test(value) ? result.push(value) : false);
  return result;
};
