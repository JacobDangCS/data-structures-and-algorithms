...
const screenForNames = (arr) => {
  let validator = /[MD][rs]+.\s[A-Za-z]+/gm;
...
let biggerThanLuke = (arr) => {
  const luke = arr.find(value => value.name === 'Luke Skywalker');
  return arr.filter(value => Number(value.mass) > Number(luke.mass))
